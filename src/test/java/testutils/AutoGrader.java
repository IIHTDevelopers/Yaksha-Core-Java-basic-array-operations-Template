...
public class AutoGrader {
    ...
    public boolean testArrayOperationsOnly(String filePath) throws IOException {
        ...
        // Check for traditional array iteration using for, while, or do-while loops
        System.out.println("------ Array Iteration ------");
        for (ForStmt forStmt : cu.findAll(ForStmt.class)) {
            // Check if the loop iterates over an array
            if (forStmt.getInitialization().toString().contains("int[]") || 
                forStmt.getUpdate().toString().contains("length")) {
        ...
        for (WhileStmt whileStmt : cu.findAll(WhileStmt.class)) {
            // Check if it's iterating through an array using a while loop
        ...
        for (DoStmt doStmt : cu.findAll(DoStmt.class)) {
            // Check if it's iterating through an array using a do-while loop
